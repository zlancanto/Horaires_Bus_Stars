...
@Entity(
    tableName = "trip",
    foreignKeys = [
        ForeignKey(entity = BusRoute::class, parentColumns = ["route_id"], childColumns = ["route_id"], onDelete = ForeignKey.CASCADE),
        ForeignKey(entity = Calendar::class, parentColumns = ["service_id"], childColumns = ["service_id"], onDelete = ForeignKey.CASCADE)
    ],
    indices = [Index("route_id"), Index("service_id")]
)
data class Trip(
    @PrimaryKey
    @ColumnInfo(name = "trip_id") val id: String,
    @ColumnInfo(name = "route_id") val routeId: String,
    @ColumnInfo(name = "service_id") val serviceId: String,
    @ColumnInfo(name = "trip_headsign") val headsign: String?,
